...
const setupTransferCommand = (data, cornodejs, vorpal) => {
  vorpal
    .command('transfer <address> <value>', 'Sends cornodes to the address')
    ...
    .action((args, callback) => {
      ...
      if (args.address.length === 90 && !cornodejs.utils.isValidChecksum(args.address)) {
        vorpal.log(chalk.red('That address appears malformed.  Please check it.'));
        return callback();
      }

      const address = args.address.length === 81
        ? cornodejs.utils.addChecksum(args.address)
        : args.address;
