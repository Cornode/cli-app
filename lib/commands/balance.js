...
// https://github.com/cornodeledger/cornode.lib.js/pull/16
const setupBalanceCommand = (data, cornodejs, vorpal) => {
  vorpal
    ...
    .action((args, callback) => {
      ...
      new Promise((resolve, reject) => {
        cornodejs.api.getNewAddress(data.seed, {returnAll: true}, (err, addresses) => {
          if (err) {
            return reject(err);
          }
      ...
      .then(addresses => new Promise((resolve, reject) => {
        cornodejs.api.getBalances(addresses, 100, (err, data) => {
          if (err) {
            return reject(err);
          }
          ...
          if (elapsedInterval) {
            ...
            vorpal.log(`Your current balance is ${chalk.yellow(collapsecornodeAmount(balance))}ι.\n`);
          }
