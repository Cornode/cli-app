...
const cornode = require('cornode.lib.js');
...
const refreshAccountData = () => {
  ...
  if (data.seed) {
    cornodejs.api.getAccountData(data.seed, (err, accountData) => {
      if (err) {
        // on fail, retry fast
...
const refreshServerInfo = () => {
  cornodejs.api.getNodeInfo((err, nodeInfo) => {
    if (err) {
      ...
      config.get('nodes', []).then(nodes => {
        const node = `${cornodejs.host}:${cornodejs.port}`.replace('http://', '');
...
setupPrompt(data, cornodejs, vorpal);
setupCommands(data, cornodejs, refreshAccountData, refreshServerInfo, vorpal);
...
vorpal.log(chalk.green(`Running cornode CLI v${version}\n`));

// Give the cornodejs connection time to settle before processing command line params
// TODO make this more deterministic.  timeouts = ugly
setTimeout(() => {
